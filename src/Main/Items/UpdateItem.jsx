...
const PizzaSchema = Yup.object().shape({
  ...
  name: Yup.string().min(5, 'Name must be 5 charaters').required('Required'),
  description: Yup.string().min(10, 'Description must be 10 charaters').required('Required'),
  price: Yup.number().min(3, 'Price must be 3 digits').required('Required').positive('Must be positive'),
  size: Yup.string().oneOf(['Small', 'Medium', 'Large'], 'Size must be small, medium, or large').required('Required'),
});
  ...
  useEffect(() => {
    const fetchPizza = async () => {
      try {
        ...
        alert('Failed to fetch pizza data. Please try again.');
      }
    };
      ...
      >
        {({ errors, touched, isSubmitting }) => (
          <Form>
            <Row className="mb-3">
              <Col>
                <Field name="image" className="form-control" placeholder="Add image" />
